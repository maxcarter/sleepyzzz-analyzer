...
module.exports = {
    ...
    update: (collection, data) => {
        log.info('Controller [database] Function [update]')
        log.info(`Updating data in Firebase [${collection}]`)
        let deferred = q.defer()
        ref.child(collection).set(data).then(() => {
            log.info(`Successfully updated data in Firebase [${collection}]`)
            deferred.resolve()
        }).catch((error) => {
            log.error(`Failed updating data in Firebase [${collection}]`, error)
